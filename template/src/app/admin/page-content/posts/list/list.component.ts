...
export class ListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.apiPath.getTypeRequest('post-tag-category').subscribe((res: any) => {
  ...
  delete(id: any, i: any, title: any) {
    ...
    if (window.confirm('Do you want to go ahead?')) {
      this.apiPath.deleteTypeRequest(`delete-post/${id}/${title}/${user[0].id}/${user[0].name}`).subscribe((res: any) => {
        if (res.status) {
